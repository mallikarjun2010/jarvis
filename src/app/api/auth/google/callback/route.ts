...
export async function GET(request: Request) {
  try {
    ...
    // Delete existing tokens for simplicity (only keep current active token)
    ...
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/?error=auth_failed&msg=${encodeURIComponent(
        error.message || ''
      )}`
    );
  }
